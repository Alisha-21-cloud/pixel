...
const Locations = ({ dateRange }: {
  ...
  const { data: geographiesData, isPending: isGeoPending } = useQuery({
    queryKey: ['geographies'],
    queryFn: async () => {
      console.log('Fetching geographies from:', geoUrl);
      const response = await fetch(geoUrl);
      if (!response.ok) throw new Error('Failed to fetch geographies');
      const data = await response.json();
      console.log('Geographies loaded successfully');
      return data;
    }
  });

  const { data, isPending } = useQuery({
    queryKey: ["locations", websiteId, dateRange],
    queryFn: async () => {
  ...
  if ((isPending || isGeoPending) && !data) {

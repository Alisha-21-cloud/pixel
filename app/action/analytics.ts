...
export async function getAnalytics(
  ...
  const startISO = (from ? startOfDay(from) : subDays(new Date(), 29)).toISOString();
  const endISO = (to ? endOfDay(to) : new Date()).toISOString();
  ...
  console.log(`Fetched ${sessions?.length || 0} sessions for ${websiteId}`)
  ...
  sessions?.forEach((session: any) => {
    ...
    const day = new Date(session.entry_time).toISOString().split("T")[0];
